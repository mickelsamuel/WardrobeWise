...
// Fully functional check for username availability (excluding current user)
export const checkUsernameAvailability = async (username) => {
  try {
    const user = auth.currentUser;
    if (!user) throw new Error('User not authenticated');
    const usersRef = collection(db, 'users');
    const q = query(usersRef, where('publicUsername', '==', username));
  ...
  try {
    ...
    snapshot.forEach(docSnap => {
      items.push({
        ...
        lastWorn: docSnap.data().lastWorn?.toDate() || null
      });
    });
  ...
  try {
    ...
    snapshot.forEach(docSnap => {
      events.push({
        id: docSnap.id,
        ...docSnap.data(),
        date: docSnap.data().date?.toDate() || null,
        createdAt: docSnap.data().createdAt?.toDate() || null
      });
    });
